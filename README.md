...
sfs --init                 # Auto-detect project and create config
...
Use `--init` to automatically detect your project settings and create a configuration file:

```bash
sfs --init
```

This analyzes your project structure and creates `.ralph/config.yaml` with detected settings:

```yaml
# .ralph/config.yaml (generated by sfs --init)
project:
  name: my-app
  language: typescript
  framework: sveltekit
...
| Language | Project File | Detected Frameworks |
|----------|--------------|---------------------|
| Node.js | package.json | Next.js, SvelteKit, React, Vue, Express, Fastify, NestJS, Nuxt |
| Python | pyproject.toml, setup.py, requirements.txt | FastAPI, Django, Flask |
| Go | go.mod | Gin, Echo, Fiber, Chi |
| Rust | Cargo.toml | Actix, Axum, Rocket, Warp |

### Init Options

```bash
sfs --init              # Interactive mode (prompts before writing)
sfs --init -y           # Non-interactive (auto-confirm)
sfs --init --force      # Overwrite existing config
sfs --init --force -y   # Overwrite without prompting
```

The init command will:
1. Detect your project's language and framework
2. Find your package manager (npm, pnpm, yarn, bun, poetry, cargo, etc.)
3. Identify test, lint, and build commands
4. Show a preview and ask for confirmation
5. Create `.ralph/config.yaml`
